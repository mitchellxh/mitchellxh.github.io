...
## Unreleased
- **Terminal bio: rewrote all rotating entries against the CV and the actual repos; corrected a factual misstatement.** The console previously rendered the NSF ACCESS award as `$3.1M HPC` and `METRICS` labelled it `grants_secured` — but it is **3,000,000 compute credits**, an allocation, not secured funding. That is now `PI · 3M NSF ACCESS credits`, with a comment in `TerminalBio.jsx` recording why it must never be written as dollars. Also corrected `12 papers` → `13` and `15+ workshops` → `20+` (the 15+ was the BU-only subtotal). Cut four of the five rotating `whoami` identities — rotating five job titles read as indecision rather than range — leaving `Data Systems & AI Engineer` as the single claim. Removed the entries that described senior work in support vocabulary (`clean, version & serve lab data`, `AI/ML + HPC research advising`, `hpc ml data enablement`, `optimizing research since 2016`) and the two that framed 13 peer-reviewed publications as abandoned (`(ex) computational scientist`, `cat past-life.txt`); the neuroscience era is now `uname -a → neuroscience -> AI infra`, a foundation rather than a past life. Added entries for work that existed but was invisible: the RAG portfolio (`epr · drylands · rao · graph` — five systems across five research domains, which is the defensible claim, where corpus size is not), URI Sheets (the deployed New Haven urban-forest survey), 1.9M trees detected across 486 NAIP images, SeedLearn's BioCLIP seedling classification, vLLM built on H200/SLURM, and 282 GB / 306k files under DVC. Every line traces to the CV or a repo with commits; forked upstreams (DeepForest, pybioclip, DeepBrainIPP, executors) and the DISSC org repos (zero commits) are deliberately excluded. 25 entries, all outputs within the component's ~34-char width budget.
- **Analytics: added the Google Analytics 4 tag (`G-5J21NQ943K`) to `index.html`, gated by a new page test.** The site now has its own GA4 property under its own Google account — no stream shared with any other property. The tag is hard-coded in the head rather than injected from a `VITE_*` env var on purpose: Vite's `htmlEnvHook` responds to a missing variable by logging a warning and leaving the literal `%VITE_GA_ID%` in the emitted HTML *with exit code 0*, so a fresh clone with no (gitignored) `.env` would silently deploy a page requesting `gtag/js?id=%VITE_GA_ID%`. The measurement id is public in view-source regardless, so indirection bought nothing and cost a silent-breakage path. Placement is the one point satisfying every constraint: after the charset meta and favicon links (so an ad blocker stalling `gtag.js` can't delay the icon) and before the theme-flash guard. Uses `window.dataLayer.push(arguments)` rather than Google's bare `dataLayer.push(arguments)` — identical in a browser, but safe for anything that evaluates the page's inline scripts outside one. No `send_page_view` wiring needed: `react-router-dom` was removed, so the single `gtag('config')` call is complete coverage.
- **Added `scripts/test_page.mjs` and wired it into `predeploy`.** Zero-dependency Node assertions over the *built* `dist/index.html`: exactly one tag loader, one matching `config` call, one `googletagmanager.com` reference, the `window.dataLayer` form present and bare `dataLayer` absent, head placement after the charset meta and ahead of the theme guard, and no foreign `G-` id (the asset-URL strip keeps Vite's base64url content hashes, which can themselves contain `G-`, from causing flaky failures). `predeploy` is now `npm run build && npm run test:page`, so a broken or unreplaced tag fails `npm run deploy` instead of shipping.
- **SEO/social: added canonical + Open Graph + Twitter Card meta to `index.html`.** Sharing `mitchellhorn.dev` now yields a rich preview (title, description, `{ HRF }` logo) instead of a bare link, and the canonical URL points to the custom domain. Also updated the plain `description` and `package.json` `homepage` to the new domain. (og:image is the square `logo512.png` for now — a dedicated 1200×630 card is a later upgrade.)
- **Custom domain: added `public/CNAME` → `mitchellhorn.dev`.** Placed in `public/` (not `dist/`) so Vite copies it to the `dist/` root on every build — a `CNAME` written directly into `dist/` would be wiped each rebuild. It rides `npm run deploy` to the gh-pages root, where GitHub Pages reads it. Not active until the next deploy lands *and* the apex A/AAAA + `www` DNS records resolve; then enable **Enforce HTTPS** (`.dev` is HSTS-preloaded → HTTPS-only).
- **Mobile: the two full-screen panels now fill the *visible* viewport (`100dvh`), fixing a strip of panel 2 bleeding into panel 1.** The panels' `min-height: 100vh` is right on desktop but on mobile `100vh` resolves against the *large* viewport (browser toolbar retracted), so as the toolbar collapses the panel ends up shorter than the live viewport and the next panel peeks in. Overriding to `min-height: 100dvh` inside the existing mobile media queries (`Content.css`, ≤900px, both `.text-section` and `.content-section`) ties each panel to the current visible height in every toolbar state. Desktop keeps `100vh` (base rule untouched) — the override is mobile-only.
- **Mobile navbar: separated the affiliation marks from the theme toggle.** The BU · Yale logos sat 8px from the fixed toggle — tighter than the 24px between the two logos, so the toggle read as a third logo. Pushed the marks left (`Navbar.css` `padding-right` 4rem → 5rem, ≤768px) and shrank the toggle below the 32px logos (`ThemeToggle.css` 2.5rem → 1.75rem, ≤768px) so it reads as a secondary control; the Yale↔toggle gap is now ~36px (measured). Mobile-only; desktop keeps the 2.5rem toggle and its own spacing.
- **Mobile CareerMap: widened the plot so date-adjacent node labels stop overlapping.** The plot already scrolls horizontally, but `min-width: 680px` was too narrow — the fixed-width (132px) date-positioned labels collided in 12 places (measured). Bumped `min-width` to `1120px` on ≤760px (`CareerMap.css`) — the SVG's native width `W`, where same-row labels clear by ~34px — so the existing horizontal scroll reveals a legible timeline instead of a squished one. Mobile-only.
...
- Navbar: dialed the frost back from `blur(48px)` to `blur(16px)` — 48px read as a heavy smear; 16px is a clean frosted-glass edge.
- Mobile hero: widened the command-line→portrait gap to `clamp(5rem, 25vh, 14rem)` (height-scaled) so the portrait drops into the lower half of the full-screen panel instead of bunching under the command line with a big void below. Verified balanced with no panel-2 overflow from iPhone SE → Pro Max.
...
- Navbar is a **fixed frosted-glass header** (`position: fixed`, heavy `backdrop-filter` blur + a light `color-mix` tint): pinned, with content blurring/softening beneath it. A pure 0-tint blur reads as invisible over a matching background, so a light material tint carries the glass; a `prefers-reduced-transparency: reduce` fallback swaps to a near-opaque bar for users whose browser disables backdrop blur (e.g. macOS "Reduce Transparency"). The blur lives on an **in-flow `position: relative` child** (`.nav-glass`) with the nav content laid over it as an absolute overlay — Chromium/WebKit composite `backdrop-filter` there, but NOT on a `position: fixed` element (nor reliably on an absolutely-positioned child), which is why it wasn't rendering. Mirrors kimsuchydesign.com.
- **Content column widened to a full-site width** — `--content-max: 1600px` + `--content-pad: clamp(1.5rem, 5vw, 5rem)` (~72px margins, calibrated against kimsuchydesign.com) now drive nav, hero, plot, and footer; was a narrow centered 1200px column that read like a word-doc.
