...
- Navbar is a **fixed frosted-glass header** (`position: fixed`, heavy `backdrop-filter` blur + a light `color-mix` tint): pinned, with content blurring/softening beneath it. A pure 0-tint blur reads as invisible over a matching background, so a light material tint carries the glass; a `prefers-reduced-transparency: reduce` fallback swaps to a near-opaque bar for users whose browser disables backdrop blur (e.g. macOS "Reduce Transparency"). The blur is applied to a `::before` layer, not the fixed element itself — Chromium/WebKit don't composite `backdrop-filter` on a `position: fixed` element (this is why it wasn't rendering).
