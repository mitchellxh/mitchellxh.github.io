...
- Navbar is now a **fixed frosted-glass header** (`position: fixed`, `backdrop-filter: blur(30px)` + a ~55% translucent `color-mix` tint): it stays pinned while content blurs and softens beneath it. A pure 0-tint blur reads as invisible over a matching background, so a light material tint carries the glass.
